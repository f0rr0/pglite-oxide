...
CONTAINER_PGVECTOR="${CONTAINER_PGVECTOR:-/work/spikes/upstream/pgvector}"
...
"$DOCKER" run --rm \
  ...
  bash -lc '
    ...
    make -s -j"$JOBS" -C /work/spikes/upstream/pgvector \
      PG_CONFIG=/work/spikes/wasix-postgres-build/pg_config_wasix.sh \
