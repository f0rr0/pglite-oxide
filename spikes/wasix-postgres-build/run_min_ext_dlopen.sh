#!/usr/bin/env bash
set -euo pipefail

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
BUILD="$ROOT/build/min_ext"

WASIX_HOME="${WASIX_HOME:-/tmp/wasixcc-home/.wasixcc}"
WASMER_BIN="${WASMER_BIN:-/tmp/wasmer-7.1/bin/wasmer}"

export HOME="${WASIX_HOME%/.wasixcc}"
export PATH="$WASIX_HOME/bin:$PATH"

"$ROOT/build_min_ext.sh" >/dev/null

wasixcc -sWASM_EXCEPTIONS=yes -sPIC=yes -sMODULE_KIND=dynamic-main \
  -Wl,-rpath,'$ORIGIN' \
  "$ROOT/min_ext/dlopen_min_ext.c" \
  -o "$BUILD/dlopen_min_ext.wasm"

"$WASMER_BIN" run "$BUILD/dlopen_min_ext.wasm" --volume "$BUILD:/lib" --cwd /lib
